...
namespace SearchService.Consumers
{
    public class AuctionUpdatedConsumer(IMapper mapper) : IConsumer<AuctionUpdated>
    {
        ...
        {
            Console.WriteLine($"Received AuctionUpdated event for Autction with ID : {context.Message.AuctionId}, Make: {context.Message.Make}, Model: {context.Message.Model}");

            var item = mapper.Map<Item>(context.Message);

            var db = await DB.InitAsync("SearchBb");
            await db.Update<Item>().Match(x => x.AuctionId == item.AuctionId)
